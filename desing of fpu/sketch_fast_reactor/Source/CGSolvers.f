      subroutine CGSolver(tol, maxits, n_iter)

c-----------------------------------------------------------------------
! driver for Conjagate Gradient Type Iterative Solvers : CG, BCGSTAB, 
!         TFQMR, FOM, GMRES)
c-----------------------------------------------------------------------
      implicit none
...
! input: 
	...
	integer n_iter
      ...
      external cg_solver, bcg, dbcg, bcgstab, tfqmr, gmres, fgmres, 
     & dqgmres
...
!      write(*,'(A,I20,/,A,I3,A,I20)' ) "Working space: BICGSTAB = ",
!     &          11*nmax,              "               GMRES(", m ,") =", 
!     &  (nmax+3)*(m+2) + (m+1)*m/2
!      write(*,'(A, F8.3, A)') "Total working area = ", lwk*8./1.E+6,
!     &   " Mb"

c
c     set the parameters for the iterative solvers
c
      ipar(1) = 0 ! Start Iterations
      ipar(2) = 2 ! Right Preconditioner (2), LEFT(1)
      ipar(3) = 2 ! Stopping Criteria
!                  || residual || <= rtol * || rhs || + atol
      ipar(4) = lwk  ! number of elements in the working array 'w'
      ipar(5) = m_gmres   ! size of the Krylov subspace
      ipar(6) = maxits ! maximum number of matrix-vector multiplies
      fpar(1) = tol ! the relative tolerance,
      fpar(2) = 1.0D-10 ! the absolute tolerance (details discussed later),
c--------------------------------------------------------------
c     read in a matrix in the SKETCH format
c--------------------------------------------------------------
!      write(*,*) 'maxits =', maxits
!      pause  
...
!         write(*,*) 'nnz = ', nnz
!       write (*,*) 'Time of Matrix Convert ', time_end - time_begin, 
!     &  ' seconds'
!      write(*,'(A, 2E12.5)')  'rhs(1), rhs(nrow) =', rhs(1), rhs(nrow)
!      write(*,'(A, 2E12.5)')  'sol(1), sol(nrow) =', sol(1), sol(nrow)
!	 pause
...
!      print *, ' '
      if(iter_solver.EQ."CG") then
            
        print *, '	*** CG ***'
        call runrc(nrow,rhs,sol,ipar,fpar,wk,xran, cg_solver) !a,ja,ia,au,jau,ju,

      else if(iter_solver.EQ."BCGSTAB") then
         print *, '	*** BCGSTAB ***'
         call runrc(nrow,rhs,sol,ipar,fpar,wk,xran, bcgstab) ! a,ja,ia,au,jau,ju,
         
      else if(iter_solver.EQ."TFQMR") then
         print *, '	*** TFQMR ***'
         call runrc(nrow,rhs,sol,ipar,fpar,wk,xran, tfqmr) !a,ja,ia,au,jau,ju,

      else if(iter_solver.EQ."FOM") then
         print *, '	*** FOM ***'
         call runrc(nrow,rhs,sol,ipar,fpar,wk,xran, fom) !a,ja,ia,au,jau,ju,

      else if(iter_solver.EQ."GMRES") then
         print *, '	*** GMRES ***'
         call runrc(nrow,rhs,sol,ipar,fpar,wk,xran, gmres) !a,ja,ia,au,jau,ju,
      else 
	  write(*,*) 'ITERATIVE  SOLVER is UNDEFINED '
        stop       
      end if
...
c-----------------------------------------------------------------------
      subroutine runrc(n,rhs,sol,ipar,fpar,wk,guess, solver) !,a,ja,ia,
!     +     au,jau,ju,solver)
      implicit none
...
c
      if (ipar(1).eq.1) then
	   call AMUX_SKETCH( wk(ipar(8)), wk(ipar(9)) )
c         write(*,*) 'n = ', n
c         write(*,*) 'ipar(8), ipar(9) =', ipar(8), ipar(9)
c         write(*,'(A, 2E12.5)') ' x(1), x(n) = ', wk( ipar(8) ),
c     &          wk( ipar(8) + n -1 )          
c         write(*,'(A, 2E12.5)') ' y(1), y(n) = ', wk( ipar(9) ),
c     &          wk( ipar(9) + n - 1 )     
c         call amux(n, wk(ipar(8)), wk(ipar(9)), a, ja, ia)
c         write(*,*) 'n = ', n
c         write(*,*) 'ipar(8), ipar(9) =', ipar(8), ipar(9)
c         write(*,'(A, 2E12.5)') ' x(1), x(n) = ', wk( ipar(8) ),
c     &          wk( ipar(8) + n -1 )          
...
c         call lusol(n,wk(ipar(8)),wk(ipar(9)),au,jau,ju)

	   call block_ssor_solve( wk(ipar(8)), wk(ipar(9)) )

         goto 10
      else if (ipar(1).eq.4 .or. ipar(1).eq.6) then
         write(*,*) 'Requested LUTSOL '
	   stop
...
!      write (iou, *) '# total execution time (sec)', time_end - time_beg
c
c     check the error
c
c      call amux(n,sol,wk,a,ja,ia)
!      call AMUX_SKETCH( sol, wk )
!      do i = 1, n
!         wk(n+i) = sol(i) -1.0D0
!         wk(i) = wk(i) - rhs(i)
!     enddo
!      write (iou, '(A, E12.5)') '# actual residual norm is', 
!     & dnrm2(n,wk,1)
!      write (iou, '(A, E12.5)') '# relative residual norm is', 
!     & dnrm2(n,wk,1)/dnrm2(n,sol,1)
!      write (iou, '(A, E12.5)') '# the error norm is', 
!     & dnrm2(n,wk(1+n),1)
c
...
c----------end-of-ddot--------------------------------------------------
      ...
      integer          next, n, nn, i, j, incx
      real   dx(1), cutlo, cuthi, hitest, sum, xmax,zero,one
...
c     CG_SOLV       -- Conjugate Gradient Method                       c
...
c
c     real function distdot(n,x,ix,y,iy)
c     integer n, ix, iy
c     real x(1+(n-1)*ix), y(1+(n-1)*iy)
c
c     This interface of DISTDOT is exactly the same as that of
c     DDOT (or SDOT if real == real) from BLAS-1. It should have
...
c     convergence tests.
c-----------------------------------------------------------------------
      subroutine cg_solver(n, rhs, sol, ipar, fpar, w)
      implicit none
      integer n, ipar(16)
      real rhs(n), sol(n), fpar(16), w(n,*)
c-----------------------------------------------------------------------
...
c-----------------------------------------------------------------------
      ...
      implicit none
      integer n, ipar(16)
      real rhs(n), sol(n), fpar(16), w(n,8)
c-----------------------------------------------------------------------
...
c
      integer i
      logical lp, rp
      real eta,sigma,theta,te,alpha,rho,tao
      save
c
...
c
      ...
      real alpha,beta,psi,c,s,distdot
      logical lp,rp,full
...
c system of equations a * sol = y.  real version.
...
c
      ...
      parameter(one=1.0D0,zero=0.0D0)
c
      real t,sqrt,distdot,ss,res,beta,ss1,delta,x,zeta,umm
      integer k,j,i,i2,ip2,ju,lb,lbm1,np,indp
      logical lp,rp,full, perm(3)
      real ypiv(3),u(3),usav(3)
      external tidycg
      save
c
c     where to go
...
c-----------------------------------------------------------------------
      ...
      real fpar(16), r(n), delx(n), sx, distdot
      external distdot
c-----------------------------------------------------------------------
...
c               -3: input vector is a linear combination of others
c
c     External routines used: real distdot
c-----------------------------------------------------------------------
      integer i,k
      real  nrm0, nrm1, fct, thr, distdot, zero, one, reorth
