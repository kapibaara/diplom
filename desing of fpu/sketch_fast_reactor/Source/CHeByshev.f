...
* Local Variables: 
     ...
     &              Volume(k)
        ...
        do k = 1, N_TOT
         do n = 1, NG
          flux_norm = flux_norm + al(n)*Flux(n,k)*Flux_a(n,k)*
     &              Volume(k)
          harm_norm = harm_norm + al(n)*Flux_k(n,k)*Flux_a(n,k)*
     &              Volume(k)
         end do
        end do
      end if
...
ccc   write(*,*) 'alfa_res = ', alfa_res
c     pause

        if(i_out.eq.0) then
c           write(*,*) 'Initial Neutron Flux: 0'
           do k = 1, N_TOT
              do n = 1, NG
*TMP                 Flux(n,k) = 0.
...
CO            Flux_k1(n, k) = ro_cheb*
CO     &        (gamma*Flux_n(n, k) + Flux_k(n, k))+cc*Flux_k1(n, k)
...
c                 write(*,*) 'i_source =', i_source , 'XME_ =', xme_
...
*npolin = -1 indicator that we start new Chebyshev sequence
                  npolin = -1
c                 write(*,*) 'NEW CHEBYSHEV SEQUENCE XME =',
c     &                               i_source, XME
              else  ! (T_CHEB /=0
...
* with initial eigenvalue estimate)
                   npolin = -1
                   xme_ = xme_ini
c                    write(*,*) 'Convergence is not optimal',
c     &                          'XME_ =', XME_
c                    pause
                else 
