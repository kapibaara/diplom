...
* seting to zero the nodal coupling coefficients
             call MAT_Set_Nodal_Coeff_Zero
...
* analytical inversion in the case of 2 neutron energy groups
	     call MAT_Inverse_2x2_Block_Diag
             ...
             IF (Scat_Matrix_Type.EQ. "TRIANG") THEN
                call MAT_Inverse_Block_Diag_TRIANGULAR
             ELSE 
...
c        write(*, *) 'xme_ =', xme_, 'npolin =', npolin
c        write(*, *) 'kin_k_ef =', kin_k_ef
c        pause
      end if
...
* reading set Nodal Coupling coefficient to ZERO

      rewind(io_unit)
      call MSC_Search_Header_In_File(io_unit, 
     &   "CNT_NOD_ZERO", input_line, fmt_inp_ident, error_find)  

      if(error_find) then
!        call MSC_ERR_Add_Message('WARNING',' Could not find '//
!    &   'identifier CNT_NOD_ZERO in the FILE_INPUT, set '//
!    &   'set macro cross section model to POLYNOM' )

        Flag_set_Nod_Coupl_Zero(:) = 1
      else

        read (io_unit,fmt=*,iostat=ios) Flag_set_Nod_Coupl_Zero(1:NG)

        call Iostat_Error_Check
     &   (ios,"error in reading Flag_set_Nod_Coupl_Zero(1:NG) "//
     &   " from the FILE_INP")
