...
!      write(*,*) 'CRD_MAT_COMP'
     ...
     &              (CR_Compos(ie, n), ie = 1, NN_CRod_El)
         write(*,*) "CRD_MAT_COMP n =", n  
         if(ios .NE. 0) then
              write(Message, '("Error Reading CR Material "//
     &               "Compositions, CR Type =", I4)' ) n
...
* Local Variables:      
c     real v_rods ! Control Rod Speed NOW IN THE GEOMETRY FILE
c      parameter( v_rods = 3. ) 
!      integer NROD1, NROD2
...
c      write(*,*) 'time = ',time,'zrods group 1 =', zrods(nrods1(1))
c      write(*,*) 'time = ',time,'zrods group 2 =', zrods(nrods2(1))
c      write(*,*) 'time_rods = ', time_rods, dt_rods
...
* Local Variables:      
c      real v_rods
c      parameter( v_rods= 150. ) ! NOW IN THE GEOMETRY FILE 
!!      real Time_Start 
...
c      write(*,*) 'time = ',time,'zrods  =', zrods(1)
c      write(*,*) 'time_rods = ', time_rods, dt_rods
