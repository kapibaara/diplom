...
c      read(io_unit,fmt=*,iostat=ios) ((npoly(i,j),j=1,NXR),i=1,NYR)
...
c*        NP_AZ -  Number of assemblies in the reactor core ! Not Used
c*        KP_AZ(NP_AZ) - Node Number of the Reactor Core ! Not Used
* local variables 
...
c              write(*,*) 'i, j =', i, j
               ...
               do j_out = 1,NPX(j)
c                  write(*,*) 'i_out, j_out =', i_out, j_out
                   nlx = nlx + 1
                   if(npoly(i,j).ne.0) then
                     np = npoly(i,j)
                     n_f = n_f + 1
c                    write(*,*) 'nlx, nly, n_f', nlx, nly, n_f
c                    pause
                     n_fine(nly, nlx) = n_f
* n_in - number of internal point into the  region
                     n_in = j_out + (i_out-1)*NPX(j)
c                    write(*,*) 'np, n_in', np, n_in
...
!        WRITE(*,*) 'NZR_Core_Beg , NZR_Core_End =', 
...
! 1st Axial Layer of the reactor core
      n1 = NZ_Core_Beg 
!+ (NZ_Core_End - NZ_Core_Beg + 1)/2

!      WRITE(*,*) 'n1 INSIDE REACTOR CORE = ', n1
!      PAUSE
...
! holes in the core
      v_core = 0.
!      write(*,*)  'NZ_Core_Beg, NZ_Core_End =',NZ_Core_Beg,NZ_Core_End
!      pause
...
*=====================================================================*
	...
	character*4 val_fmt
	character*4 val_char(0:N_POLY)
	character*5 val_char_5(0:N_POLY)
...
! "    Numbering the Reactor Bundles (Assemblies):"	 
	...
	ind = 0
	  ...
	  DO j=1,NXR
	     IF(npoly(i,j) .NE. 0) THEN
	       ind = ind + 1
             write(val_char_5(ind), '(I5)')  npoly(i,j)
           END IF
         END DO
      END DO
      call OUT_Write_Map(N_POLY,  NXR_B_Min_Reactor, 
     &   NXR_Max_Reactor, NXR_B_Reactor, NXR_E_Reactor, NYR_B_Reactor,
     &   NYR_E_Reactor, npoly, Header_Map, unit, val_char_5, val_fmt)
...
! "    Bundle Material Compositions:"      
     ...
     &  "    Bundle Material Compositions:"
      DO ind = 1, N_Bundle_Type
         write(unit, ' (8x, I3,":", 10I7, /, 12x, 10I7,/, 12x, 10I7)')
...
!left
                    IF( nlx==1 .OR. n_fine(nly,nlx-1)==0 ) THEN
	                 la(n_f,1) = I_BOUND_NODE
                    ELSE
                       la(n_f,1) = n_fine(nly,nlx-1)
                    END IF 
* up
                    IF(nly==1 .OR. n_fine(nly-1,nlx)==0) THEN
...
* hexagonal         
     ...
     &                    n_fine(nly-1, nlx-1)==0 ) THEN
                    ...
                    END IF 
* right
                    IF(nlx==NX.OR.n_fine(nly,nlx+1)==0) THEN
...
* down
                    ...
                    IF(nly==NY.OR.n_fine(nly+1,nlx)==0) THEN
