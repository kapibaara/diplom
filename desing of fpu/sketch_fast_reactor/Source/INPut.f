...
* Neutron Group Order  of the Source Iterations
      ...
      if( (Time + dt_sketch).gt.TTV(NP_VIEW) ) then
         write(*,*) 'Please, Check value of the Time Transient:
...
c           write(*,*) 'eigenv_shift =', eigenv_shift
c           pause
...
!=============================================================================!
     ...
     *   FILE_INPUT,  File_Reference,
     &   File_Map, File_Dist, File_CD, FILE_CD_REFL, File_BRN_In,
...
!------------------------------MISHA------------------------------------------!
      NAMELIST /INF/
     &  NMF10,NMF11,NMF12,NMF13,NMF14,NMF15,NMF16,NMDOP,NMF17,NMFBJ
!------------------------------END_MISHA--------------------------------------!
...
!      OPEN( io_unit, FILE='D_HEX26',STATUS='OLD')
!      READ(io_unit,NML = INF)
!      close(io_unit)
...
!end_MISHA
         ...
         IF(Problem_Type.EQ."Kinetics") THEN
            IF(FILE_DMP_IN.EQ.FILE_DMP_OUT_KIN) THEN
               WRITE(Message, '(A)') 
     &       "Name of the Input Restart File FILE_DMP_IN ="// 
     &       " Name of the Output Restart File FILE_DMP_OUT_KIN. "//
     &       "They should be different in Kinetics Calculations "//
     &       " Please, change FILE_DMP_OUT_KIN in SKETCH.INI"
              CALL MSC_ERR_Add_Message('ERROR',Message)
	      END IF
          END IF
