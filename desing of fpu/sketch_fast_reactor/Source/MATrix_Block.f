      subroutine MAT_Set_Nodal_Coeff_Zero
*=====================================================================*
*   Set Matrix Nodal Coupling Coefficient to Zero                     *
*=====================================================================*	   
      USE GeoMeTry_Faces  
      implicit none    
...
* Local Variables:
      ...
      DO n = 1, NG 
         IF ( Flag_set_Nod_Coupl_Zero(n) == 0 ) THEN
           MAT_Nod(n,:) = 0.0
         END IF
      END DO
...
* Steady-State and Kinetics Diffusion Calculations
      ...
      if(Problem_Type.EQ."Kinetics") then
        do n = 1, NG
          Time_Der(n) = al(n)/dt_kin
c             Time_Der(n) = al(n)*(1./dt_kin + Point_Deriv)
        end do
      else
         do n = 1, NG
            Time_Der(n) = 0.
     ...
     &                       eigenv_shift*xpn(n)*XS_SF(m,k)
              ...
              MAT_Block_Diag(n,n,k) = MAT_Block_Diag(n,n,k) + 
     &              MAT_Diag_TOT(n,k) + Time_Der(n)*volume(k)
           end do
        end do
...
! Preparing the Matrices
	...
	real dt_kin
	real Time_Der(NG), Mat_Block_Diag_Output(NG, NG)
