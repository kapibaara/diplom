...
ccc   real const_sq
ccc	parameter (const_sq = 0.822782)
	logical flag_hex
...
!                END IF
     ...
     &                fm0, hk, d2,  sq, flux_surface, a_rl, fg(1,1,nd), 
...
* calculation of the nodal coupling coefficients
     ...
     &            mat_fd(1,nl), current, fm0, sq, a_rl, mat_nod(1,nl))
             ELSE IF( NonlinearIterations.EQ."Moon") THEN
              ...
              CALL MAT_Compute_Coupling_Coeff_Bound_Moon(NG, current,
     &                fm0, hk, d2,  sq, flux_surface, a_rl, fg(1,2,nd), 
...
c      real am2_check(NG,NG), x1_check(NG)


c     write(*,*) 'Coeff_14 =', Coeff_14
c     write(*,*) 'Coeff_1c =', Coeff_1c
c     pause
...
c      write(*,*) 'am2_check =', am2_check
c      write(*,*) 'x1_check =', x1_check
c      write(*,*) 'b8_l =', b8_l
...
!	          flux_surface(n)=
!     &           (2*cg_nod(2, nd)*(d2(n)-D_Nod(n,k,2,nd))*fm0(n))/
!     &           ( 2*cg_nod(2, nd)*
!     &           (D_Nod(n,k,2,nd)+d2(n))+fg(n,2,nd)*hk_fd)
!                write(*,*) 'RIGHT Flux Surface =', flux_surface(n) 
!                write(*,*) 'RIGHT current =', current(n)
!               current(n) =  ( MAT_FD(n, nl) - MAT_NOD(n, nl))*fm0(n) 
!               write(*,*) 'RIGHT current =', current(n)/sq
!                pause 

! checking     
!                write(*,*) 'LEFT Flux Surface =', flux_surface(n) 
!	           flux_surface(n)=-(2*cg_nod(1, nd)*
!     &           (D_Nod(n,k,1,nd)-d2(n))*fm0(n))/
!     &         (2*cg_nod(1, nd)*(D_Nod(n,k,1,nd)+d2(n))+fg(n,1,nd)*hk)
!                write(*,*) 'LEFT Flux Surface =', flux_surface(n) 
!                pause 
!                write(*,*) 'LEFT current =', current(n)
!               current(n) =  -( MAT_FD(n, nl) + MAT_NOD(n, nl))*fm0(n) 
!               write(*,*) 'LEFT current =', current(n)/sq
!                pause 
      RETURN
	END
