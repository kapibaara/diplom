...
SUBROUTINE Input_Geometry_Data
   ...
   USE Formats, ONLY : Flag_Core_Absorber_Included 
   ...
   IF ( Flag_Core_Absorber_Included ) THEN
   WRITE(o_unit, '(/,A,/)') 'Reactor Core Includes Regions with Pure Absorber'
   ELSE
   WRITE(o_unit, '(/,A,/)') &
      'Reactor Core DOES NOT Includes Regions with Pure Absorber'
   END IF
...
SUBROUTINE Read_Model_parameters(i_unit)
   ...
   USE Distributions, ONLY : flag_reactor_st_dist, flag_reactor_tr_dist
...
! Steady-State Data
   ...
   IF (.NOT. ALLOCATED(flag_reactor_st_dist)) ALLOCATE (flag_reactor_st_dist(N_ST_DIST))
   ...
   DO n = 1,  n_st_dist
	   READ(i_unit) flag_reactor_st_dist(n)
   END DO
...
!
   ...
   IF (.NOT. ALLOCATED(flag_reactor_TR_dist)) ALLOCATE (flag_reactor_TR_dist(N_TR_DIST) )
   ...
   END DO
	...
	DO n = 1,  n_tr_dist
	   READ(i_unit) flag_reactor_TR_dist(n)
   END DO
...
!               e-mail: slava@lstf3.tokai.jaeri.go.jp                         !
!=============================================================================!
...
! Flag_Core_Absorber_Included                                      .True.     !  
!=============================================================================!
...
! Computing Average & Maximum Values
      DO i = 1, N_TR_DIST
         IF( flag_reactor_tr_dist(i) ) THEN
         CALL Compute_3D_Average(dist_tr(0,0,i), 1, NZR, &
            npoly, dist_tr_mm(3,i), dist_tr_mm(-3,i),    &
            k_dist_tr_mm(1,3,i), k_dist_tr_mm(1,-3,i) )      
         CALL Compute_2D_Average(dist_tr(0,0,i), 1, NZR, &
            npoly, dist_tr_mm(2,i), dist_tr_mm(-2,i),    &
            k_dist_tr_mm(1,2,i), k_dist_tr_mm(1,-2,i) )      
         CALL Compute_1D_Average(dist_tr(0,0,i), 1, NZR, &
            npoly, dist_tr_mm(1,i), dist_tr_mm(-1,i),    &
...
!            WRITE(*,*) 'FMT_TI_R =', fmt_ti_r
         ...
         CASE( "D" ) 
            ...
            CALL Write_Dist_TM(unit, Name_TR_Dist(i_out_ti(n)),              &
                 ...
                 n_digit_out, fmt_out, flag_reactor_tr_dist(i_out_ti(n)) )
...
SUBROUTINE  Write_Dist_TM(unit, name_dist,                     &
                 ...
                 n_digit_out, fmt_out, flag_reactor_dist)
   ...
   INTEGER,       INTENT(IN) :: k_dist_mm(3,-3:3)
   LOGICAL,       INTENT(IN) :: flag_reactor_dist
  
! LOCAL
   INTEGER                    :: n, ns, NZ_Start, NZ_End
   ...
   IF(flag_reactor_dist) THEN
   CALL OUT_Write_2D_Map(N_POLY, NXR, NYR, NXR_B_Min_Reactor,            &
         NXR_Max_Reactor, NXR_B_Reactor, NXR_E_Reactor, NYR_B_Reactor,   &
         NYR_E_Reactor,  npoly, Header, unit, val_char, n_digit_out)
   ELSE
   ...
   IF(flag_reactor_dist) THEN
      ...
      NZ_End = NZR
   ELSE 
      NZ_Start= NZR_Core_Beg
      NZ_End =  NZR_Core_End 
   END IF 

   DO ns = NZ_Start, NZ_End
...
!          nlz_core( ns ) , dist(0,ns)*scale_factor
   ...
   IF(TM_3D_Output == "YES") THEN
     ...
     CALL Write_3D_Dist_HORIZ(Header, unit, dist(0,0), scale_factor, &
        n_digit_out, fmt_out, flag_reactor_dist)
   END IF
...
SUBROUTINE OUT_Write_2D_Map(N_POLY_L, NXR_L, NYR_L, NXR_Beg_Min, NXR_Max,    &
           NXR_Beg, NXR_End, NYR_Beg, NYR_End, npoly_l,  Header_Map, io_out, &
           Value, n_digits)
!=====================================================================!
...
!               e-mail: slava@lstf3.tokai.jaeri.go.jp                 !
!=====================================================================!
      USE Model_Parameters, ONLY : gmt_crd_type
      IMPLICIT NONE
! Input:
      INTEGER N_POLY_L, NYR_L, NXR_L
      INTEGER NXR_Beg_Min, NXR_Max, NXR_Beg(NYR_L), NXR_End(NYR_L), &
           NYR_Beg, NYR_End, npoly_l(NYR_L, NXR_L)
...
! Local Varia      
      CHARACTER*3 val_fmt
      INTEGER  N_Position, N_Blank, nly, nlx, N_Blank_HEX
...
! Local:
   ...
   INTEGER N_XY_PLANE, NZ_Start, NZ_End
   ...
   IF(flag_reactor_dist) THEN
      ...
      NZ_End     =  NZR
   ELSE
      N_XY_PLANE = NP_Reactor_Core
      NZ_Start   = NZR_Core_Beg
      NZ_End     = NZR_Core_End 
   END IF 

   N_Cycle = N_XY_PLANE/n_data_in_line + 1
   N_Left = MOD(N_XY_PLANE, n_data_in_line)
...
!            scale_factor, no=1, N_OUT)
     IF(flag_reactor_dist) THEN
      WRITE(unit, FMT = fmt_wr)   ns,                                &
            (dist(no+(nc-1)*n_data_in_line, ns)*                     &
            scale_factor, no=1, N_OUT)
      ELSE
      WRITE(unit, FMT = fmt_wr)   ns,                                &
            (dist(Numb_Reactor_Core(no+(nc-1)*n_data_in_line), ns)*  &
...
! Preparing Average and Maximum Data
      DO i = 1, N_ST_DIST
         IF( flag_reactor_st_dist(i) ) THEN
         CALL Compute_3D_Average(dist_st(0,0,i), 1, NZR, &
              npoly, dist_st_mm(3,i), dist_st_mm(-3,i),  &
              k_dist_st_mm(1,3,i), k_dist_st_mm(1,-3,i) )      
         CALL Compute_2D_Average(dist_st(0,0,i), 1, NZR, &
            npoly, dist_st_mm(2,i), dist_st_mm(-2,i),    &
            k_dist_st_mm(1,2,i), k_dist_st_mm(1,-2,i) )      
         CALL Compute_1D_Average(dist_st(0,0,i), 1, NZR, &
            npoly, dist_st_mm(1,i), dist_st_mm(-1,i),    &
      ...
      SELECT CASE(type_out_st(n))
         CASE( "D" ) 
            ...
            CALL Write_Dist_TM(unit, Name_ST_Dist(i_out_st(n)),              &
                 ...
                 n_digit_out, fmt_out, flag_reactor_st_dist(i_out_st(n)) )
...
SUBROUTINE Write_3D_Dist_HORIZ(Header, unit, dist, scale_factor, &
              n_digit_out, fmt_out, flag_reactor_dist)
...
!   END IF
   ...
   N_Out = NZ_End - NZ_Start + 1

   WRITE(char_n_out, '(I2)') n_out

   fmt_wr = "(/, A5,"//char_n_out//"I"//char_n_digit//")"
   WRITE(unit, FMT = fmt_wr)  " CHAN",                     &
                  (no, no=NZ_Start, NZ_End)
!      WRITE(*, FMT = fmt_wr)  " CHAN",                     &
!                  (no+(nc-1)*n_data_in_Line, no=1, N_OUT)
      fmt_wr = "(5x, "//char_n_out//'(" ",'//char_n_digit1//'("-") ) )'
!      WRITE(*, FMT=fmt_wr) 
   ...
   fmt_wr = '(1x, I3, ":", '//char_n_out//TRIM(fmt_out(2:))
     DO np = 1, N_XY_PLANE
         IF(flag_reactor_dist) THEN
           WRITE(unit, FMT = fmt_wr)   np,                                &
            (dist(np, ns)*scale_factor, ns=NZ_Start, NZ_End)
      ELSE
           WRITE(unit, FMT = fmt_wr)   np,                                &
            (dist(Numb_Reactor_Core(np), ns)*scale_factor,                &
            ns=NZ_Start, NZ_End)
      END IF
      END DO
   RETURN
END SUBROUTINE Write_3D_Dist_HORIZ
