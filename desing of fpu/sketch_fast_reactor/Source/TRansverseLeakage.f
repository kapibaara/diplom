      ...
      if(Problem_Type.EQ."Kinetics") then
       ...
       Adjoint = .False.
        ...
        do k = 1, N_TOT
          a_vol = 1./volume(k)
          do n = 1, NG
          kin_trl(n, k) = al(n)*Flux(n,k)/dt_kin
...
*=====================================================================*
     ...
     &               status ='unknown')
        do nd = NDD, NDD
           ...
           write(io_unit,1) (TRL_QLA(1, n, Numb(k,nd), nd),k=1,NZ)
           write(io_unit,*) '2nd Expansion Coefficient'
           write(io_unit,1) (TRL_QLA(2, n, Numb(k,nd), nd),k=1,NZ)
       end do ! NDD
      close(io_unit) 
...
c              if(nd.eq.3.and.kt.le.NH) then	  
c               write(*,*) 'k =', k, 'kt =', kt
c               write(*,*) 'k_left =', k_left, 'k_right=', k_right
c               write(*,*) 'd2 =', d2
c               pause
c              end if  
...
c                 write(*,*) 'trl_s(i) =', (trl_s(i),i=1, n_interface)
c           pause
...
c                   trl_qla(1,n,k1,nd) = 0.5*(trl_s(i+1) - trl_s(i))
c                   trl_qla(2,n,k1,nd) = 0.5*(trl_s(i+1) + trl_s(i)) -
c     &                          trl0_xyz(n,k1,nd)
...
c     &   AAA(3,NXYZ_MAX+1), res(NXYZ_MAX+1)
* Used Subroutines: 
*                  progonka
...
c     &                          trl0_xyz(n,k1,nd)
c                   write(*,*) 'nd =', nd, 'k1 =', k1
c                   write(*,*) 'trl_s(i), trl_s(i+1) =', trl_s(i), 
c     &                                 trl_s(i+1)
c                   write(*,*) 'trl0 =', trl0_xyz(n,k1,nd)
c                   pause
!                 if(nd.eq.3.and.kt1.le.NH) then	  
!                   write(*,*) 'i =', i
!                   write(*,*) 'kt1=', kt1
