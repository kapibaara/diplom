...
c   For the Coupling without Restart	
      Logical No_Restart
c      parameter (NO_Restart = .True.)
...
cc                            if(dt_trac .LT. dt_sketch) dt_sketch = dt_trac
...
c           write(*,*) 'PVM_SKETCH_Receive',  'i_trac_end =', i_trac_end
...
c       write(*,*) 'time_diff, time_diff/dt_step =', time_diff, 
c     &                                             time_diff/dt_step
        write(*,*)'TIME STEP SIZE =', dt_sketch, dt_step
c     &     'TE_Flux =', TE_FLUX
c       write(*,*) 'dt_step =', dt_step 
c     pause
...
c      do k = 1, N_TOT
c         do n = 1, NG
c            Eps = Flux(n, k) - Flux_Coarse(n, k)
c            Flux_Norm = Flux_Norm + Flux(n, k)*Flux(n, k)
c            TE_Flux = TE_Flux + Eps*Eps
c
c         end do
c      end do
c            TE_Flux = sqrt(TE_Flux)/sqrt(Flux_Norm)

      do n1 = 1, NZ      
         do k = 1, NH
            Eps  = p(k,n1) - Pow_Coarse(k,n1) 
            Flux_Norm = Flux_Norm + p(k,n1)*p(k,n1)
            TE_Flux = TE_Flux + Eps*Eps
         end do
      end do


c     write(*,*) 'TE_Flux = ', TE_Flux
c     write(*,*) 'Flux_Norm =', Flux_Norm
c     pause
...
c      write(*,*) 'dt_new =', dt_new, 'dt_step =', dt_step
c      write(*,*) 'dt_factor =', dt_factor
c      pause
