...
!External Function
      ...
      real XS_INTER(N_XS_TAB), k_ef_norm
...
c      write(*,*) 'NZ_CORE_BEG, NZ_COE_END =', NZ_CORE_BEG, 
c     &          NZ_CORE_END 
...
c           write(*,*) 'Rodded Node = k, kc, ir, n1, icr =', 
c     &           k, kc, ir, n1, icr
c           pause
...
* thermal absorption
     ...
     &                xappa_rod(2))
              ...
              sect_rod =  XS_INTER(6)*vol*k_ef_norm
              XS_SF(1, kt) = XS_Sect_CR(sect_nrod, sect_rod, 
     &         xappa_rod(1))

              sect_nrod = XS_SF(2, kt)
              sect_rod =  XS_INTER(7)*vol*k_ef_norm
...
!      if(Debug) call XS_DBG_output
     ...
     &   XS_D, XS_SIK, XS_SA, XS_SF, XS_SF_P, k_ef_norm)
*=====================================================================*
...
* Absorption + Scattering
      ...
      XS_SF(1) = XS_INTER(6)*vol*k_ef_norm
      XS_SF(2) = XS_INTER(7)*vol*k_ef_norm
...
c         write(*,*) 'k, n1, kc, nc =', k, n1, kc, nc
c         write(*,*) 'NZ_Coe_Beg, NZ_Core_End =', NZ_Core_Beg, 
c     &    NZ_Core_End
c         pause
...
c      if(Comp_Ref_Xenon) then
!        Power_Table = Powden(ift)*U_Weight(ift)
c         Power_Table = Powden(ift)*U_Weight(ift)*25.
c      end if

c      write(*,*) 'Power_Table =', Power_Table
c	pause

      call XSR_Compute_XS_XE(N_XS, burnup, void, Xe, 
     &       Sigma_Xe(1, ift), XS_INTER, Comp_Ref_Xenon)

c      XS_SA1_OLD = XS_INTER(4)
      call XSR_Compute_XS_Doppler( Fuel_temp, ift, Burnup, 
     &                   Void, XS_INTER )
c      Delta_SA1 = (XS_INTER(4)/XS_SA1_OLD - 1.)*100
c      write(*,*) 'Doppler Effect in SA1, SA1_OLD, SA1, Change (%)'
c      write(*,*)  XS_SA1_OLD, XS_INTER(4), Delta_SA1
c      read(*,*)       
...
! Local Variables:
      character*6 Sub_Titl(6)      
c      data Sub_Titl / "FTYP  ", "POWDEN", "TEMP  ",
c     &   "DOPPLE", "SIGAXE", 	 "SSHPOL" / 
      integer i
...
c      write(*,*) Fuel_Temp, Temp_Fuel_Nom(ift)
c	read(*,*) 
...
!      "D1", "D2", "SR1", "SA1", "SA2", "nuSF1"  "nuSF2", "NY"
!        1    2      3      4      5      6        7      8   
c      data  NAME_FDCR /'DSA2  ','DNSF2 '/
c      data NAME_DCR / "DD1   ", "DD2   ", "DSIGR1 ", "DSA1  ",
c     &  "DNSF1 ", "DNY   " /
! Input:
      integer N_XS
...
c      write(*,*) 'Sigma_Micro_Xenon = ', Sigma_Micro_Xenon
c      write(*,*) 'Xe concentration = ', Xe
c      pause
c      write(*,*) 'Power Table =', power_table*648
c	pause
...
c      write(*,*) 'Sigma_Micro_Xenon = ', Sigma_Micro_Xenon
c      write(*,*) 'Xe concentration = ', Xe
c      write(*,*) 'Reference Xe concentration = ', Xenon_Ref
