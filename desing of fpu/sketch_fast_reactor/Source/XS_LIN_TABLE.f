C *******************************************************************
C *   INTERPOLATION PARAMETER'S LIBRARY READING                     *
C *******************************************************************
      SUBROUTINE YMNCFALI(fullfilename)
 
      implicit      none
      include       '../Include/YMNC_DAT.FH'
...
C____ ERROR FLAG FOR OBJECT YMNC
      YMNCerr = 0
 
C____ OPENING LIBRARY BINARY FILE FOR READING
      libuni0 = 1
      open(libuni0,file=fullfilename,form='binary',status='old')
 
C____ READING LIBRARY VERSION IDENTIFIER:
      read(libuni0) version
      if(version.ne.'NF201') YMNCerr = 1
 
C____ READING TOTAL NUMBER OF ARGUMENTS:
      read(libuni0) narg
      if(narg.ne.YMNCnarg) YMNCerr = 2
 
C____ READING TOTAL NUMBER OF INTERPOLATION PARAMETERS:
      read(libuni0) nnf
      if(nnf.ne.YMNCnfnum) YMNCerr = 3
 
C____ READING TOTAL NUMBER OF PARAM'S SORTS:
      read(libuni0) nsort
      if(nsort.ne.YMNCnsort) YMNCerr = 4
 
C____ READING COEFFICIENTS ARRAY TOTAL DIMENSION FOR ONE SORT:
      read(libuni0) ksdimtotal
      if(ksdimtotal.ne.YMNCksdimtotal) YMNCerr = 5
 
C____ READING INTERP. COEF. NUMBER OF EXT. MULT. FACTOR:
      read(libuni0) nextmult
 
C____ READING INTERP. POINTS NUMBER FOR EACH ARGUMENT:
      read(libuni0) YMNCnnodd
 
C____ READING PARAMETER'S ARGUMENTS NUMBER SUBJECT TO REDUCING:
      read(libuni0) YMNCredarg
 
C____ READING INPUT REDUCING VALUES(0-NO REDUCING;!=0-REDUCING PRESENCE):
      read(libuni0) YMNCreduce
 
C____ READING (ARGUMENT POINTS NUMBER-1) OR (1) IF ARGUMENT IS REDUCED:
      read(libuni0) YMNCnnodd1
 
C____ READING INTERP. PARAMETER'S OFFSETS ARRAY IN THE COEFF.ARRAY:
      read(libuni0) YMNCksoff
 
C____ READING COEFFICIENTS DIMEENSION ARRAY FOR EACH INTERP. PARAM.
      read(libuni0) YMNCksdimarr
 
C____ READING KOEFF. "2**ARG.NUM." FOR EACH INTERP. PARAM.
      read(libuni0) YMNCksnargarr
 
C____ READING SORT OFFSETS ARRAY
      read(libuni0) YMNCsortoff
 
C____ READING SORT INDEXES ARRAY
      read(libuni0) YMNCsortind
 
C____ READING 1./GRID STEP FOR EACH ARGUMENTS:
      read(libuni0) YMNCostep
 
C____ READING MINIMUM ARGUMENTS VALUES:
      read(libuni0) YMNCfval
 
C____ READING MAXIMUM ARGUMENTS VALUES:
      read(libuni0) YMNClval
 
C____ READING POINTS VALUES
      read(libuni0) YMNCuzlval
 
C____ READING NUMBER OF DERIVATIVES FOR EACH PARAMETER
      read(libuni0) YMNCdfnum
 
C____ READING DERIVATIVE'S POSITION FOR EACH PARAMETER
      read(libuni0) YMNCdflag
 
C____ READING EXTERNAL MULT. FACT. EXISTS FLAG VALUE:
      read(libuni0) YMNCemfl
 
C____ READING INTERNAL MULT. FACT. EXISTS FLAG VALUE:
      read(libuni0) multint
 
C____ READING STEP FLAG
      read(libuni0) YMNCstepfl
 
C____ READING TOTAL COEFFICIENTS ARRAY FOR ALL SORTS
      read(libuni0) YMNCksarr
 
C____ READING EXTERNAL MULT.FACTORS IF THEY EXISTS:
      if(YMNCemfl.ne.0) then
        read(libuni0) YMNCextmultarr
      end if
 
C____ READING INTERNAL MULT.FACTORS IF THEY EXISTS:
      if(multint.ne.0) then
        read(libuni0) YMNCintmultarr
      end if
 
C____ CLOSING LIBRARY BINARY FILE
      close(libuni0)
 
      RETURN
      END
C *******************************************************************
C *   PARAMETER'S INTERPOLATION FOR CURRENT PARAMETER'S SORT        *
C *******************************************************************
...
C____ CURRENT INTERVAL ARRAY CALCULATION
...
C____ PROCCESSING FOR ALL INTERPOLATION PARAMETERS:
      do i = 1,YMNCnfnum
 
        ksdfa(i) = (i-1)*YMNCnarg
 
C____ FORMING INTERVAL ARR.(NFINT),ARG. OFFSETS ARR.(NFXOF)
C____ AND INTERP.PNT.NUM.-1 ARR.(NPNT_1) FOR CURRENT INTERP. PARAMETER
        k=0
        do j = 1,YMNCnarg
...
C____ CALCULATION RESULT FOR ARGUMENTS NUMBER == 0
          k = YMNCsortoff(YMNCsortind(nsort)) + YMNCksoff(i) + 1
          resul(i) = YMNCksarr(k)
        else
C____ DEFINE INTERPOLATION FUNCTION AND RECEIVE RESULT
C____ FINDING OFFSET IN THE K_ARR SUBJECT TO INTERVAL ARRAY
          k  = nfint(1)
...
C____ CALLING INTERPOLATION FUNCTION
