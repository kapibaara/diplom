...
* reading XS_MODL_TYPE
     ...
     &      (xs_model .NE. "XS_BN800")  .AND.	   
     &      (xs_model .NE. "TABLE") ) THEN
            WRITE(Message, '(A,A,A)') 
     &       "XS model =",
     &         xs_model, 
     &       " in FILE_INP,  known types 'POLYNOM', "//
     &       " SUBSET, SUBSED, XS_BN800 and 'TABLE' 'LINTAB' "
...
!      END IF
	ELSE IF( XS_MODEL.eq."XS_BN800" ) THEN

         WRITE(*,*) 'BN 800 XS MODEL'  

         CALL XS_init_lib_bn800( file_CD )
!----------------------------------------------------------------
!         WRITE(*,*) 'BN 800 XS MODEL END'  
!         READ(*,*)
     ...
     &    ' assembly  discontinuity factors set to 1')
      ...
      read(io_unit,fmt=*, iostat=ios) NNODE_ADF
      DO k = 1, NNODE_ADF
* input data for 2 group energy case
         read(io_unit,fmt=*, iostat=ios) k_adf(k)
         DO n = 1, NG
          read(io_unit,fmt=*, iostat=ios) 
     &           (( adf(k_adf(k), n, i, nd), i = 1,2), nd = 1, NDD)
         END DO
      END DO           
     ...
     &     " from the FILE_INPUT file ")' ) k
       ...
       WRITE(*,*) 'READING ADF IS DONE'


      ELSE ! XS_MODEL /= SUBSET AND XS_MODEL /= LINTAB AND /=XS_BN800
...
*=====================================================================*
	...
	REAL, PARAMETER :: D1_multiplier = 0.9
      ...
      if(XS_Model .EQ. "POLYNOM".OR. XS_model.EQ. "XS_BN800") then
	  call XSP_Compute
...
!        IF ( Flag_change_D1 ) THEN
!          CALL XS_Multiply_SF1_CR( D1_multiplier )
!        END IF         
...
c      open(io_unit,file = 'Output_Debug/CRD_Xappa.dat', 
c     &      status = 'unknown')
c        write(io_unit,'(A)') 'Weighting factors for Partially'// 
c     &             ' Rodded Nodes'
c	  write(io_unit, '(A)') ' rod number, rodded part, xappa(NG)'
c        do ir = 1, NN_CROd
c	    write(io_unit, '(I4, 3E12.5)') ir, CRD_h_rod_output(ir),
c     &     (CRD_Xappa_OUtput(n, ir), n = 1, NG)
c        end do
...
!         input data for 2 group energy case
     ...
     &           d_out(k,2), sa(k,2),    sf(k,2),   sf_p(k,2), k
         ...
         ELSE IF( XS_MODEl /= "XS_BN800") THEN
! TMP FOR BN-600
      	   WRITE(unit, '(A,I4)') 
...
! "    Assembly Discontinuity Factors"
      IF(Flag_ADF) THEN  
      IF( XS_MODEL == "XS_BN800" ) THEN  
      WRITE(unit, '(A)') 
     &  "    Assembly Discontinuity Factors: Left, Right, Up, Down,"//
     &       "Bottom, Top)"
      WRITE(io_unit,'(A,I5)') 
     &  'Number of Materials with ADF not equal to 1 :', 
     &     NNODE_ADF
      DO k = 1, NNODE_ADF
      	   WRITE(unit, '(A,I4)') 
     &  "           Material Composition number :", k_adf(k)
* input data for 2 group energy case
	   DO n = 1, NG
